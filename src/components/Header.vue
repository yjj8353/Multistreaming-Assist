<template>
  <q-header elevated>
    <!-- 상단 바 -->
    <q-bar data-tauri-drag-region class="q-electron-drag">
        ...
        <q-menu>
          <q-list v-if="isNginxRunning" dense style="min-width: 100px">
            <q-item clickable v-close-popup>
              <q-item-section @click="nginxProcessCheck"
                >NGINX 프로세스 확인</q-item-section
              >
            </q-item>
          </q-list>
      ...
      <div class="q-ml-md cursor-pointer non-selectable">
        ...
        <q-menu>
          <q-list dense style="min-width: 100px">
            ...
            <q-item clickable v-close-popup>
              <q-item-section @click="contributors"
                >도움을 주신분들</q-item-section
              >
            </q-item>
          </q-list>
        </q-menu>
    ...
    <div class="bg-blue-6">
      <q-tabs>
        <q-route-tab label="스트리밍" to="/" exact />
        <q-route-tab label="녹화" to="/recording" exact />
      </q-tabs>
    </div>
  </q-header>
</template>
...
export default {
  ...
  mixins: [CheckMixin, StoreMixin, ElectronMixin],
  ...
  methods: {
    ...
    async handleResize() {
      this.isMaximized = await this.isWindowMaximized();
    },
  },
};
</script>
